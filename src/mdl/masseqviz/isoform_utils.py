...
def get_feature_interval_list(
    gene_db: gffutils.interface.FeatureDB, included_feature_types: List[str]
) -> List[Interval]:
    ...
    feature_interval_list = sorted(
        feature_interval_list, key=lambda interval: interval.start
    )
...
def fetch_alignments(
...
) -> Tuple[List[pysam.AlignedSegment], Dict[str, Any]]:
    ...
    num_total_alignments = sum(
        list(
            len(alignment_list)
            for alignment_list in query_name_to_alignment_list_map.values()
        )
    )
    ...
    for query_name, alignment_list in query_name_to_alignment_list_map.items():
        ...
        map_qualities = [
            ...
            if record.mapping_quality < max_mapping_quality
            else -np.inf
        ...
        if len(alignment_list) == 1:
            if map_qualities[0] < min_mapping_quality:
                ...
                if aln.reference_start >= (
                    fetch_start - gene_body_padding
                ) and aln.reference_end <= (fetch_end + gene_body_padding):
            ...
            if all(map_quality < min_mapping_quality for map_quality in map_qualities):
                ...
                if (
                    sum(
                        ...
                        for map_quality in map_qualities
                    )
                    > 1
                ):
...
def get_overlap_fraction(
    ref_start: int, ref_end: int, test_start: int, test_end
) -> float:
...
def get_vectorized_repr(
    ...
    for i, vectorization_interval in enumerate(vectorization_interval_list):
        ...
        for block in record.get_blocks():
            ...
            vectorization_interval_length = (
                vectorization_interval.end - vectorization_interval.start + 1
            )
    ...
    if vectorization_strategy == "relative":
        return np.clip(rel_overlap_vector, 0.0, 1.0)
    elif vectorization_strategy == "absolute":
        return abs_overlap_vector
    elif vectorization_strategy == "quantized":
        quantized_vectorization_repr = (
            abs_overlap_vector > abs_overlap_quantization_threshold
...
def get_binary_reference_coverage(
    record: pysam.AlignedSegment, ref_interval_start: int, ref_interval_end: int
) -> np.ndarray:
    ...
    for block in alignment_block_list:
        block_start = (
            min(max(block[0], ref_interval_start), ref_interval_end)
            - ref_interval_start
        )
        block_end = (
            max(min(block[1], ref_interval_end), ref_interval_start)
            - ref_interval_start
        )
...
def get_binary_reference_coverage_sparse_matrix(
    ...
    coverage_matrix_csr_ni = sp.coo_matrix(
        ...
        shape=(len(alignment_list), ref_interval_end - ref_interval_start + 1),
    ).tocsr()
...
def get_vectorized_alignments_matrix(
    ...
    vectorized_alignments_nv = np.zeros(
        (len(alignment_list), len(vectorization_interval_list))
    )
    for i_record, record in enumerate(alignment_list):
        vectorized_alignments_nv[i_record, :] = get_vectorized_repr(
            record,
...
def get_feature_to_annotation_track_map(
    ...
    for feature_type in included_feature_types:
        feature_to_annotation_track_map[feature_type] = np.zeros(
            (ref_interval_end - ref_interval_start + 1,), dtype=np.int8
        )
    for feature in feature_interval_list:
        if feature.contig == ref_contig:
            feature_overlap_start = (
                min(max(feature.start, ref_interval_start), ref_interval_end)
                - ref_interval_start
            )
            feature_overlap_end = (
                max(min(feature.end, ref_interval_end), ref_interval_start)
                - ref_interval_start
            )
            feature_to_annotation_track_map[feature.feature_type][
                feature_overlap_start : (feature_overlap_end + 1)
            ] = 1
...
def get_junctions_and_counts_from_alignments(
    ...
    consider_read_endpoints_as_junctions: bool,
) -> Tuple[List[int], List[int]]:
    """Counts junctions for a given list of gapped alignments."""
    junction_counter = defaultdict(int)
    for alignment in alignment_list:
        normalized_blocks = merge_alignment_blocks(
            alignment.get_blocks(), alignment_block_min_gap
        )
        ...
        for i, block in enumerate(normalized_blocks):
            if i == 0 and not consider_read_endpoints_as_junctions:  # first block
                ...
                i == (len(normalized_blocks) - 1)
                and not consider_read_endpoints_as_junctions
            ):  # last block
                junction_counter[block[0]] += 1
            else:
                junction_counter[block[0]] += 1
                junction_counter[block[1]] += 1
    sorted_junctions_and_counts = sorted(
        list(junction_counter.items()), key=lambda x: x[0]
    )
...
def merge_alignment_blocks(
    raw_alignment_blocks: List[Tuple[int, int]], alignment_block_min_gap: int
) -> List[Tuple[int, int]]:
    ...
    for raw_block in raw_alignment_blocks[1:]:
        ...
        assert (
            curr_start >= prev_end
        ), "The blocks must be non-overlapping and coordinate sorted!"
...
def merge_junctions(
    junctions: List[int], counts: List[int], junction_merge_gap: int
) -> Tuple[List[int], List[int]]:
        ...
        mean_merged_junction = int(
            np.round(
                sum(
                    j * c
                    for j, c in zip(
                        curr_junction_cluster_positions, curr_junction_cluster_counts
                    )
                )
                / total_counts
            )
...
def filter_junctions(
    ...
    min_abs_junction_read_support: int,
) -> Tuple[List[int], List[int]]:
    """Filters out low frequency junctions."""
    min_abs_junction_read_support = min(
        min_abs_junction_read_support, total_reads * min_rel_junction_read_support
    )
    passing_junctions = [
        junctions[idx]
...
def get_alignment_bounds(
    raw_alignment_list: List[pysam.AlignedSegment],
) -> Tuple[int, int]:
...
def get_vectorization_interval_list_from_features(
    feature_interval_list: List[Interval],
    gene_id: str,
    min_vectorization_interval_length: int,
) -> List[Interval]:
    ...
    for i in range(len(feature_endpoints) - 1):
        ...
        if end - start + 1 >= min_vectorization_interval_length:
            vectorization_interval = Interval(
                f"vectorization_interval_{gene_id}", contig, start, end
            )
...
def get_vectorization_interval_list_from_junctions(
    ...
    if include_end_segments:
        first_segment = Interval(
            f"vectorization_interval_{gene_id}",
            reference_contig,
            reference_start,
            junctions[0],
        )
        if (
            first_segment.end - first_segment.start + 1
        ) >= min_vectorization_interval_length:
    ...
    if include_end_segments:
        ...
        if (
            last_segment.end - last_segment.start + 1
        ) >= min_vectorization_interval_length:
...
def get_flank_augmented_binary_array(binary_array_mv: np.ndarray) -> np.ndarray:
    ...
    flank_augmented_binary_array_mv = (
        binary_array_mv
        + (np.arange(vector_sz)[None, :] < first_nnz_idx_m[:, None]).astype(
            binary_array_mv.dtype
        )
        + (np.arange(vector_sz)[None, :] > last_nnz_idx_m[:, None]).astype(
            binary_array_mv.dtype
        )
    )
...
def mutual_support_l1_distance(x, y, metric_alpha):
    ...
    return (
        metric_alpha * distance_in_mutual_support + (1.0 - metric_alpha) * distance_raw
    )
...
def get_degradation_aware_similarity(
    ...
    return np.exp(
        -exp_scale
        * get_degradation_aware_distance(
            x,
            y,
            segment_lengths,
            penalty_5p_per_bp,
            penalty_3p_per_bp,
            penalty_mutual_support_per_bp,
        )
    )
